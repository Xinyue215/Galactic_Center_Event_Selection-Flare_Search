...
def submit_jobs(N, njob, threshold, cut_n_sigma, cpus):

	T = time.time()

	job_base_dir = ensure_dir(f'/scratch/{username}/GC_Untriggered_Flare_Search/bg_trials_{T:17.6f}')
	...
	commands = []
	labels = []
	bg_trials = os.path.abspath('bg_trials_old.py')
	for i in range(njob):
		fmt = "{} --N {} --label {} --threshold {} --cut_n_sigma {} --cpus {}"
		commands.append(fmt.format(bg_trials, N, i, threshold, cut_n_sigma, cpus))
...
if __name__ == "__main__":
	...
	submit_jobs(args.N, args.njob, args.threshold, args.cut_n_sigma, args.cpus)
